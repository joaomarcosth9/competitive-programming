...
struct Seg {
    int N;
    vector<int> seg;
    Seg(int n) : N(n), seg(2 * N) { }
    void update(int x) {
        seg[x += N]++;
        for (x >>= 1; x > 0; x >>= 1) seg[x] = seg[x << 1] + seg[x << 1 | 1];
    }
...
void solve() {
    ...
    Seg segL(n + 1), segR(n + 1);

    ll lower = 0;
    
    for (int i = n; i >= 1; i--) {
        if (a[i] < mx) lower++;
        cont[i] += segR.query(0, a[i] - 1);
        segR.update(a[i]);
        inv += cont[i];
    }
    for (int i = 1; i <= n; i++) {
        cont[i] += segL.query(a[i] + 1, n);
        segL.update(a[i]);
    }

    res[mx] = lower * (lower + 1) / 2;

    debug(res[mx]);

    for (int i = mx - 1; i >= 1; i--) {
        res[i] = res[i + 1];
        for (auto u : idx[i]) res[i] -= u;
        for (auto u : idx[i + 1]) res[i] += n - u + 1;
    }
    ...
    for (int i = 1; i <= n; i++) {
        debug(i);
        cout << inv - cont[i] << " " << res[a[i]] << endl;
        cout << (((inv - cont[i]) * n + res[a[i]]) * overn) << " ";
        cout << endl;
    }
    cout << endl;
