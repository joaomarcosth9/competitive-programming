...
const int maxn = 2e5 + 5, maxa = 2e6 + 5;
...
void sieve() {
    ...
    for (long long i = 2; i < maxa; i++) {
        if (is_prime[i]) {
            for (long long j = i * i; j < maxa; j += i) is_prime[j] = false;
        }
    }
}
...
void solve() {
    ...
    for (int i = 0; i < 2 * n; i++) {
        if (is_prime[a[i]]) b.eb(a[i]);
        else nfreq[a[i]]++, cntnot++;
    }
    ...
    debug(over);
    dp[cntnot] = fat[n] / over;

    for (auto [u, f] : freq) {
        vector<mint> ndp(2 * n + 1);
        debug(dp);
        for (int i = 0; i <= 2 * n; i++) {
            if (i + f <= 2 * n) ndp[i + f] += dp[i] * ifat[f];
            if (f > 0 && i + f - 1 <= 2 * n) ndp[i + f - 1] += dp[i] * ifat[f - 1];
        }
        debug(ndp);
        dp = ndp;
    }
...
signed main() {
    ...
    for (int i = 0; i < maxa; i++) is_prime[i] = 1;
    sieve();
