...
ll all(ll k) { // todos k primeiros bits acesos
    return (1LL << k) - 1;
}

ll first(ll i, ll k) { // primeiro cara >= 2^i que tem popcount == k
    return (1LL << i) + all(k - 1);
}
...
void solve() {
    ...
    while (n >= 1 && k >= 0) {
        ...
        if (k >= lg) { // contar a "juncao" dos ranges
            mint add = min(first(lg, k + 1), n);
            res += (add - p2[lg]) * p2[lg];
        }
...
int32_t main() {
    ...
    for (int i = 2; i <= 62; i++) {
        ...
        for (int j = 1; j <= 62; j++) {
            if (j >= i) {
                dp[i][j] = p2[i] * (p2[i] + 1) * inv2; // tudo de [0, 1, ..., n-1]
            } else if (j + 1 == i) {
                dp[i][j] = (p2[i] - 1) * (p2[i] + 1 - 1) * inv2;
            } else { // j + 1 < i
