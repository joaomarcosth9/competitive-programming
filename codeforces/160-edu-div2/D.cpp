...
struct SparseTable {
    ...
    vector<vi> st;
    SparseTable() { }
    SparseTable(vi &v) : n(v.size()), e(32 - __builtin_clz(n)) {
        st.assign(e + 1, vi(n));
...
mint solve(int l, int r, int dir) {
    if (l > r) return 1;
    auto [val, id] = st.query(l, r);
    mint L = solve(l, id - 1, dir | 1);
    mint R = solve(id + 1, r, dir | 2);
    mint ret = L * R;
    debug(l, r, L, R, ret);
    if (dir & 1) ret += L;
    debug(dir & 1, ret);
...
void solve() {
    ...
    auto L = solve(0, id - 1, 1);
    auto R = solve(id + 1, n - 1, 2);
    debug(L, R);
