...
template <typename T> struct sms {
    ...
    friend void swap(sms &a, sms &b) { swap(a.root, b.root), swap(a.N, b.N); }
    ...
    node *insert(node *at, T idx, T l, T r) {
        ...
        T m = l + (r - l) / 2;
        if (idx <= m)
            at->l = insert(at->l, idx, l, m);
        else
            at->r = insert(at->r, idx, m + 1, r);
        return at->update(), at;
    }
    void insert(T v) {
        assert(v >= 0);
    ...
    node *erase(node *at, T idx, T l, T r) {
        ...
        if (l == r)
            ...
            T m = l + (r - l) / 2;
            if (idx <= m)
                at->l = erase(at->l, idx, l, m);
            else
                at->r = erase(at->r, idx, m + 1, r);
            at->update();
        }
        return at;
    }
    ...
    node *split(node *&x, int k, T tam) {
        ...
        if (tam == 1)
            ...
            if (k <= count(x->l, tam >> 1))
                ret->l = split(x->l, k, tam >> 1);
            else {
