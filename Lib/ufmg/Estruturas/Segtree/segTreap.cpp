...
template <typename T> struct treap {
    struct node {
        ...
        pair<ll, ll> idx; // {y, x}
        T val, mi;
        node(ll x, ll y, T val_) : l(NULL), r(NULL), p(rng()), idx(pair(y, x)), val(val_), mi(val) {}
        void update() {
            mi = val;
            if (l) mi = op(mi, l->mi);
            if (r) mi = op(mi, r->mi);
    ...
    void split(node *i, node *&l, node *&r, pair<ll, ll> idx) {
        ...
        if (i->idx < idx)
            split(i->r, i->r, r, idx), l = i;
        else
            split(i->l, l, i->l, idx), r = i;
        i->update();
    }
...
template <typename T> struct segtreap {
    ...
    T query(ll lx, ll rx, ll ly, ll ry, int p, ll l, ll r) {
        ...
        ll m = l + (r - l) / 2;
        return op(query(lx, rx, ly, ry, get_ch(p, 0), l, m), query(lx, rx, ly, ry, get_ch(p, 1), m + 1, r));
    }
    ...
    void update(ll x, ll y, T val, int p, ll l, ll r) {
        ...
        ll m = l + (r - l) / 2;
        if (x <= m)
            update(x, y, val, get_ch(p, 0), l, m);
        else
            update(x, y, val, get_ch(p, 1), m + 1, r);
