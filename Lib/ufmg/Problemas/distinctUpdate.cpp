...
namespace bit {
    ...
    void build() {
        ...
        for (int i = 1; i <= n; i++) {
            int j = i + (i & -i);
            if (j <= n)
                for (auto x : bit[i]) bit[j].insert(x);
        }
    }
    ...
    int query(int l, int r, int x) { return pref(r + 1, x) - pref(l, x); }
...
void build() {
    ...
    sort(t.begin(), t.end());
    for (int i = 0; i < n; i++) {
        if (i and t[i].first == t[i - 1].first) prv[t[i].second] = t[i - 1].second;
        if (i + 1 < n and t[i].first == t[i + 1].first) nxt[t[i].second] = t[i + 1].second;
    }
...
int query(int a, int b) { return b - a + 1 - bit::query(a, b, b + 1); }
