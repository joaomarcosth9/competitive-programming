...
struct QuadEdge {
    ...
    QuadEdge(int id_ = -1, pt o_ = pt(INF, INF)) : id(id_), o(o_), rot(nullptr), nxt(nullptr), used(false) {}
...
pair<Q, Q> build_tr(vector<pt> &p, int l, int r) {
    ...
    while (true) {
        if (ccw(lb->o, ra->o, ra->dest()))
            ra = ra->rev()->prev();
        else if (ccw(lb->o, ra->o, lb->dest()))
            lb = lb->rev()->next();
        else
            break;
    }
    ...
    while (true) {
        Q L = b->rev()->next();
        if (valid(L))
            while (in_c(b->dest(), b->o, L->dest(), L->next()->dest())) del_edge(L, L->next());
        Q R = b->prev();
        if (valid(R))
            while (in_c(b->dest(), b->o, R->dest(), R->prev()->dest())) del_edge(R, R->prev());
...
vector<vector<int>> delaunay(vector<pt> v) {
    ...
    if (col) {
        for (int i = 1; i < n; i++) g[idx[i - 1]].push_back(idx[i]), g[idx[i]].push_back(idx[i - 1]);
        return g;
    }
