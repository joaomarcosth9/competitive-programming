...
template <typename T> struct ETT {
    ...
    struct node {
        ...
        node(int id_, T v, bool f_ = 0)
            : l(NULL), r(NULL), p(NULL), pr(rng()), sz(1), val(v), sub(v), lazy(), id(id_), f(f_), qt_f(f_) {}
        void prop() {
            if (lazy != T()) {
                if (f) val += lazy;
                sub += lazy * sz;
    ...
    void join(node *l, node *r, node *&i) { // assume que l < r
        ...
        l->prop(), r->prop();
        if (l->pr > r->pr)
            join(l->r, r, l->r), l->r->p = i = l;
        else
            join(l, r->l, r->l), r->l->p = i = r;
        i->update();
    }
    ...
    pair<int, int> get_range(int i) { return {get_idx(first[i]), get_idx(last[i])}; }
