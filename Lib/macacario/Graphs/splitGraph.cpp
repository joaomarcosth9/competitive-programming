...
class SplitGraph {
  ...
  public:
    ...
    vector<int> getCliqueSet() { return K; }

    vector<int> getStableSet() { return I; }

    bool isSplit() { return maxClique >= 0; }

    int maximumClique() { return maxClique; }
    ...
    void extend(int c) {
        ...
        if (p == -1)
            ...
            if (len[p] + 1 == len[q])
                ...
                int clone = newnode();
                len[clone] = len[p] + 1;
                for (int i = 0; i < ALFA; i++) nxt[clone][i] = nxt[q][i];
                link[clone] = link[q];
                cnt[clone] = 0;
                while (p != -1 && nxt[p][c] == q) {
                    nxt[p][c] = clone;
