...
namespace lp {
    ...
    void init(int nvars, double *f, int flag) {
        ...
        for (int i = 0; i < n; i++) ar[0][i] = f[i] * minmax_flag;
    }
    ...
    void init() {
        ...
        for (int j = 0; j <= n; j++) ar[0][j] = -ar[0][j], index[j] = j, val[j] = 0;
    }
    ...
    inline double solve() {
        ...
        while (true) {
            ...
            for (j = 0, l = 0; j < n; j++)
                if ((ar[k][j] < (ar[k][l] - EPS)) ||
                    (ar[k][j] < (ar[k][l] - EPS) && index[i] < index[j] && (rand() & 1)))
                    l = j;
            if (ar[k][l] >= -EPS) {
        ...
        while (true) {
            ...
            if (ar[0][l] > -EPS) break;
            for (i = 1, k = 0; i <= m; i++)
                if (ar[i][l] > EPS && (!k || ar[i][n] / ar[i][l] < ar[k][n] / ar[k][l] - EPS ||
                                       (ar[i][n] / ar[i][l] < ar[k][n] / ar[k][l] + EPS && basis[i] < basis[k])))
                    k = i;
            if (ar[k][l] <= EPS) {
