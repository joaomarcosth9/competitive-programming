...
struct cont {
    cont *ls, *rs;
    int x;
    cont(int _x, vector<cont *> &gc) : x(_x), ls(NULL), rs(NULL) { gc.push_back(this); }
    cont(cont *l, cont *r, vector<cont *> &gc) : x(0), ls(l), rs(r) { gc.push_back(this); }
};
...
class Deque {
  ...
  public:
    ...
    ~Deque() {
        ...
        for (int i = 0; i < (int)conts.size(); i++) delete conts[i];
    }
