...
bool collinear(point p, point q, point r) { return fabs(cross(p - q, r - p)) < EPS; }
...
bool between(point p, point q, point r) { return collinear(p, q, r) && inner(p - q, r - q) <= 0; }
...
bool isConvex(polygon &P) {
    ...
    for (int i = 1; i < n; i++) {
        if (ccw(P[i], P[(i + 1) % n], P[(i + 2) % n]) != left) return false;
    }
    return true;
}
    ...
    while (scanf("%d", &na), na) {
        ...
        if (caseNo == 19) {
            ...
            for (int i = 0; i < int(pq.size()); i++) {
                if (fabs(pq[i].second) > EPS) printf("%d %.2f\n", pq[i].first, pq[i].second);
            }
        printf("\n");
    }
    return 0;
