...
struct truple {
    int a, b, c;
    truple(int _a, int _b, int _c) : a(_a), b(_b), c(_c) {
        if (a > b) swap(a, b);
        if (a > c) swap(a, c);
        if (b > c) swap(b, c);
    }
...
namespace Delaunay {
    ...
    void brute(int l, int r) {
        ...
        for (int u = l; u <= r; u++)
            for (int v = u + 1; v <= r; v++) add_edge(u, v);
        if (r - l == 2) tri.insert(truple(l, l + 1, r));
    }
    double theta[MAXN];
    bool comp(int u, int v) { return theta[u] < theta[v]; }
    vector<vector<int>> g;
    bool right;
    point base;
...
bool between(point p, point q, point r) { return collinear(p, q, r) && inner(p - q, r - q) < -EPS; }
