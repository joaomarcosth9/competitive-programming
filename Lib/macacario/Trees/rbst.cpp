...
struct node {
    int v, sum, size;
    node *l, *r;
    node(int _v, vector<node *> &nodes) : v(_v), sum(_v), size(1), l(NULL), r(NULL) { nodes.push_back(this); }
};
...
void insert(vector<int> &arr, int i, int v) { arr.insert(arr.begin() + i, v); }
...
void erase(vector<int> &arr, int i) { arr.erase(arr.begin() + i); }
