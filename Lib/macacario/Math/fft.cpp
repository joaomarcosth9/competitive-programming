...
void sqrtConv(vector<ll> a, vector<ll> b, vector<ll> &c) {
    ...
    for (int l = 0; l < 2; l++)
        for (int r = 0; r < 2; r++) convolution(ca[l], cb[r], cc[l][r]);
    c.resize(cc[0][0].size());
    for (int i = 0; i < (int)c.size(); i++) {
        c[i] = (((ll)round(cc[1][1][i].real())) % MOD * (SMOD * SMOD) % MOD) % MOD +
               ((ll)round(cc[0][1][i].real())) % MOD * SMOD % MOD + ((ll)round(cc[1][0][i].real())) % MOD * SMOD % MOD +
