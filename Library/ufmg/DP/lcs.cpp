...
void dp_top(int li, int ri, int lj, int rj) {
    ...
    for (int i = li; i <= ri; i++) {
        for (int j = rj; j >= lj; j--)
            dp[0][j - lj] = max(dp[0][j - lj], (lcs_s[i] == lcs_t[j]) + (j > lj ? dp[0][j - 1 - lj] : 0));
        for (int j = lj + 1; j <= rj; j++) dp[0][j - lj] = max(dp[0][j - lj], dp[0][j - 1 - lj]);
    }
}
...
void dp_bottom(int li, int ri, int lj, int rj) {
    ...
    for (int i = ri; i >= li; i--) {
        for (int j = lj; j <= rj; j++)
            dp[1][j - lj] = max(dp[1][j - lj], (lcs_s[i] == lcs_t[j]) + (j < rj ? dp[1][j + 1 - lj] : 0));
        for (int j = rj - 1; j >= lj; j--) dp[1][j - lj] = max(dp[1][j - lj], dp[1][j + 1 - lj]);
    }
}
