...
vector<pt> convex_hull(vector<pt> v) { // convex hull - O(n log(n))
    ...
    vector<pt> l, u;
    for (int i = 0; i < v.size(); i++) {
        while (l.size() > 1 and !ccw(l.end()[-2], l.end()[-1], v[i])) l.pop_back();
        l.push_back(v[i]);
    }
    for (int i = v.size() - 1; i >= 0; i--) {
        while (u.size() > 1 and !ccw(u.end()[-2], u.end()[-1], v[i])) u.pop_back();
        u.push_back(v[i]);
    }
    l.pop_back();
    u.pop_back();
...
struct convex_pol {
    ...
    int extreme(const function<bool(pt, pt)> &cmp) {
        ...
        while (l + 1 < r) {
            ...
            if ((!last_dir and cur_dir) or (last_dir == cur_dir and rel_dir == cur_dir)) {
                l = m;
                last_dir = cur_dir;
            } else
                r = m;
...
bool operator==(const line &a, const line &b) { return !(a < b) and !(b < a); }
...
struct cmp_sweepline {
    bool operator()(const line &a, const line &b) const {
        ...
        if (!eq(a.p.x, a.q.x) and (eq(b.p.x, b.q.x) or a.p.x + eps < b.p.x)) return ccw(a.p, a.q, b.p);
...
struct cmp_sweepangle {
    bool operator()(const line &a, const line &b) const { return get_t(dir, a) + eps < get_t(dir, b); }
};
