...
bool blank(char c) { return c == ' '; }

bool is_unary(char c) { return c == '+' or c == '-'; }
...
int eval(string &s) {
    ...
    for (int i = 0; i < s.size(); i++) {
        ...
        if (s[i] == '(') {
            ...
            while (op.size() and ((!r_assoc(o) and priority(op.top()) >= priority(o)) or
            ...
            int val = 0;
            while (i < s.size() and isalnum(s[i])) val = val * 10 + s[i++] - '0';
            i--;
            st.push(val);
            un = false;
        }
