...
void multievaluate(int u, double x[], double y[], int i, int j) {
    if (i == j) {
        y[i] = 0;
        double p = 1;
        for (int k = 0; k < (int)ep[u].size(); k++) y[i] += ep[u][k].real() * p, p *= x[i];
        return;
    }
...
void interpolate(vector<base> &a, int u, double x[], int n) {
    ...
    y[2 * u + 1].resize(m);
    for (int i = 0; i < m; i++) y[2 * u + 1][i] = (y[u][n / 2 + i] - ya0[i]) / yp[i];
    interpolate(a1, 2 * u + 1, x + (n / 2), m);
    convolution(p, a1, a);
    int sz = max(a.size(), a0.size());
    a.resize(sz);
    for (int i = 0; i < sz && i < n / 2; i++) a[i] += a0[i];
    a.resize(n);
}
