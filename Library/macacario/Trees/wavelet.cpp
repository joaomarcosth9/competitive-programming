...
#define right(p) ((p << 1) + 1)
    ...
    int rank(int u, int lo, int hi, int q, int i) {
        ...
        if (q <= mid)
            return rank(left(u), lo, mid, q, ri);
        else
            return rank(right(u), mid + 1, hi, q, i - ri);
    }
    ...
    int quantile(int u, int lo, int hi, int i, int j, int k) {
        ...
        if (k <= c)
            return quantile(left(u), lo, mid, ri + 1, rj, k);
        else
            return quantile(right(u), mid + 1, hi, i - ri, j - rj, k - c);
    }
    ...
    void swap(int u, int lo, int hi, int v1, int v2, int i) {
        ...
        if (v1 <= mid) {
            if (v2 > mid)
                ...
                swap(left(u), lo, mid, v1, v2, ml[u][i]);
        } else {
            if (v2 <= mid)
                ml[u][i]++;
            else
                swap(right(u), mid + 1, hi, v1, v2, i - ml[u][i]);
        }
    }
