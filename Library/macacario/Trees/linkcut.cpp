...
class LinkCutTree {
    ...
    void refresh(node *p) { p->sw = p->w + sum(p->ls) + sum(p->rs); }
    ...
    void splay(node *p) {
        ...
        while (p->par != NULL) {
            ...
            if (q == r->ls) {
                if (p == q->ls)
                    rotateright(q), rotateright(p);
                else
                    rotateleft(p), rotateright(p);
            } else {
                if (p == q->rs)
                    rotateleft(q), rotateleft(p);
                else
                    rotateright(p), rotateleft(p);
            }
        }
        refresh(p);
    }
  ...
  public:
    ...
    bool IsSameTree(int u, int v) { return findroot(u) == findroot(v); }
    int depth(int u) {
        access(&lct[u]);
...
class UndirectedLinkCutTree {
  ...
  public:
    ...
    bool IsSameTree(int u, int v) { return lct.IsSameTree(u, v); }
};
