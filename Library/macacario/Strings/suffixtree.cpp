...
struct node {
    ...
    node(int l = 0, int r = 0, int par = -1) : l(l), r(r), par(par) { memset(next, -1, sizeof next); }
    ...
    state go(state st, int l, int r) {
        while (l < r) {
            if (st.pos == tree[st.v].len()) {
                ...
                if (text[tree[st.v].l + st.pos] != text[l]) return state(-1, -1);
                if (r - l < tree[st.v].len() - st.pos) return state(st.v, st.pos + r - l);
    ...
    void pop(int pos) {
        ...
        while (tree[leaf].nsons == 0) {
            if (ptr.v != leaf) {
                ...
                if (ptr.pos == min(tree[leaf].r, pos) - tree[leaf].l) break;
                int mid = split(ptr);
                ptr.v = mid;
                num_substr -= min(tree[leaf].r, pos) - tree[leaf].l;
