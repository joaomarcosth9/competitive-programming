...
struct point {
    ...
    point normalized() { return point(x, y, z) * (1.0 / norm()); }
    ...
    point operator+(point other) const { return point(x + other.x, y + other.y, z + other.z); }
    point operator-(point other) const { return point(x - other.x, y - other.y, z - other.z); }
    point operator*(double k) const { return point(x * k, y * k, z * k); }
};

double dist(point p1, point p2) { return (p1 - p2).norm(); }

double inner(point p1, point p2) { return p1.x * p2.x + p1.y * p2.y + p1.z * p2.z; }
...
bool collinear(point p, point q, point r) { return cross(p - q, r - p).norm() < EPS; }

double angle(point a, point o, point b) { return acos(inner(a - o, b - o) / (dist(o, a) * dist(o, b))); }
...
point distVectorBasePoint(line l, point p) { return proj(p - l.r, l.v) + l.r; }

point distVectorEndPoint(line l, point p) { return p; }
...
double dist(line a, line b) { return distVector(a, b).norm(); }
...
point distVectorEndPoint(line a, line b) { return distVectorBasePoint(a, b) + distVector(a, b); }